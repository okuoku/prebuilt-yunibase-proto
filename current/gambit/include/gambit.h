...
#if ___VERSION != 409004
#include "gambit-not409004.h"
#else
...
#ifdef ___DONT_INLINE_JUMPS
#error "Define either ___INLINE_JUMPS or ___DONT_INLINE_JUMPS"
#endif
#else
#ifndef ___DONT_INLINE_JUMPS
#if __llvm__ || __clang__
#define ___DONT_INLINE_JUMPS
#endif
#endif
#endif
...
#if __has_builtin(builtin_setjmp) || (defined(__GNUC__) && ___GNUC_VERSION > 500000)
#undef ___USE_builtin_setjmp
#define ___USE_builtin_setjmp
#endif

#ifdef ___DONT_USE_builtin_setjmp
#undef ___USE_builtin_setjmp
#endif

#ifdef ___USE_builtin_setjmp
#define ___SETJMP(jmpbuf)__builtin_setjmp (jmpbuf)
#define ___LONGJMP1(jmpbuf)__builtin_longjmp (jmpbuf,1)
#else
#define ___SETJMP(jmpbuf)setjmp (jmpbuf)
#define ___LONGJMP1(jmpbuf)longjmp (jmpbuf,1)
#endif

#if defined(setjmp) || defined(___USE_builtin_setjmp)
#undef ___CAN_IMPORT_SETJMP_DYNAMICALLY
#define ___CAN_IMPORT_SETJMP_DYNAMICALLY
#endif
...
#ifdef ___PROPER_TAIL_CALL
#define ___TAIL_CALL_VOID(call) ___PROPER_TAIL_CALL(call)
#define ___TAIL_CALL_NONVOID(call) ___PROPER_TAIL_CALL(call)
#else
#define ___TAIL_CALL_VOID(call) do { call; return; } while (0)
#define ___TAIL_CALL_NONVOID(call) return call
#endif

#undef ___USE_PROPER_TAIL_CALL_EXCLUSIVELY_TO_EXIT_HOST
#undef ___USE_TAIL_CALL_TO_EXIT_HOST

#if !defined(___CENTRALIZED_TRAMPOLINE) && defined(___USE_SETJMP) && (defined(___PROPER_TAIL_CALL) || !defined(__llvm__))
#define ___USE_TAIL_CALL_TO_EXIT_HOST
#endif

#if defined(___PROPER_TAIL_CALL) && defined(___USE_TAIL_CALL_TO_EXIT_HOST) && !defined(___SUPPORT_MULTIPLE_C_COMPILERS)
#define ___USE_PROPER_TAIL_CALL_EXCLUSIVELY_TO_EXIT_HOST
#endif
...
#ifdef ___DONT_USE_builtin_expect
#undef ___USE_builtin_expect
#endif
...
#undef ___USE_builtin_op_overflow
#define ___USE_builtin_op_overflow
#undef ___USE_builtin_popcount
#define ___USE_builtin_popcount
#undef ___USE_builtin_ffs
#define ___USE_builtin_ffs
#undef ___USE_builtin_clrsb
#define ___USE_builtin_clrsb
#endif
#endif
...
#undef ___USE___builtin_readcyclecounter
#define ___USE___builtin_readcyclecounter
#endif
#endif

#ifdef ___DONT_USE_builtin_op_overflow
#undef ___USE_builtin_op_overflow
#endif

#ifdef ___DONT_USE_builtin_popcount
#undef ___USE_builtin_popcount
#endif

#ifdef ___DONT_USE_builtin_ffs
#undef ___USE_builtin_ffs
#endif

#ifdef ___DONT_USE_builtin_clrsb
#undef ___USE_builtin_clrsb
#endif
...
#ifdef ___DONT_USE___builtin_readcyclecounter
#undef ___USE___builtin_readcyclecounter
#endif
...
#define ___FETCH_WORD(base,i)*(___CAST(___WORD*,base)+(i))
#define ___STORE_WORD(base,i,val)*(___CAST(___WORD*,base)+(i)) = (val)
...
#define ___FETCH_C(s,i)___CS_SELECT( \
___FETCH_U8(s,i), \
___FETCH_U16(s,i), \
___FETCH_U32(s,i))
#define ___STORE_C(s,i,val)___CS_SELECT( \
___STORE_U8(s,i,val), \
___STORE_U16(s,i,val), \
___STORE_U32(s,i,val))
...
#define ___FIXMULPCONSTY_NOTFALSEP(var,x,y)___FIXMULP_NOTFALSEP(var,x,y)
#else
#define ___FIXMULP_NOTFALSEP(var,x,y)(___temp=___FIXWRAPMUL(x,y),___EXPECT_FALSE(___FIXQUO(___temp,y)!=(x))?0:(var=___temp,1))
#define ___FIXMULPCONSTY_NOTFALSEP(var,x,y)(___temp=___FIXWRAPMUL(x,y),___EXPECT_FALSE(___FIXQUOCONSTY(___temp,y)!=(x))?0:(var=___temp,1))
#endif
#define ___FIXMULP(x,y)(___FIXMULP_NOTFALSEP(___temp,x,y)?___temp:___FAL)
#define ___FIXMULPCONSTY(x,y)(___FIXMULPCONSTY_NOTFALSEP(___temp,x,y)?___temp:___FAL)
...
#define ___FIXWRAPQUOCONSTY(x,y)((y)==___FIX(-1)?___FIXWRAPNEG(x):___FIXQUOCONSTY(x,y))

#if ___WORD_WIDTH == 32 || defined(___HAVE_FAST_DIV64)

#define ___FIXQUO(x,y)___FIX(((x)/(y)))
#define ___FIXQUOCONSTY(x,y)___FIXQUO(x,y)

#define ___FIXREM(x,y)___CAST(___WORD,(x)%(y))
#define ___FIXREMCONSTY(x,y)___FIXREM(x,y)

#else

/*
 * Typical processors have a 32 by 32 integer division instruction
 * that is considerably faster than the 64 by 64 integer division
 * instruction.  The following macros will dynamically check the
 * arguments of the division to determine if a 32 by 32 division will
 * work and use that when possible.  To always use the 64 by 64 bit
 * division, define ___HAVE_FAST_DIV64 when compiling the C code.
 */

#define ___S64_FITS_IN_S32(x)(___CAST(___UWORD,((x)>>31)+1)<2)

#define ___FIXQUO(x,y) \
(___S64_FITS_IN_S32(x) && ___S64_FITS_IN_S32(y) \
 ?___FIX(___CAST(___WORD,___CAST(___S32,x)/___CAST(___S32,y))) \
 :___FIX((x)/(y)))
#define ___FIXQUOCONSTY(x,y) ___FIX((x)/(y))

#define ___FIXREM(x,y) \
(___S64_FITS_IN_S32(x) && ___S64_FITS_IN_S32(y) \
 ?___CAST(___WORD,___CAST(___S32,x)%___CAST(___S32,y))  \
 :___CAST(___WORD,(x)%(y)))
#define ___FIXREMCONSTY(x,y) ___CAST(___WORD,(x)%(y))

#endif

#define ___FIXMOD(x,y) \
((___temp=___FIXREM(x,y)) != ___FIX(0) && \
  ((x)>>(___WORD_WIDTH-1)) != ((y)>>(___WORD_WIDTH-1)) \
 ?___FIXADD(___temp,y) \
 :___temp)
#define ___FIXMODCONSTY(x,y) \
((___temp=___FIXREMCONSTY(x,y)) != ___FIX(0) && \
  ((x)>>(___WORD_WIDTH-1)) != ((y)>>(___WORD_WIDTH-1)) \
 ?___FIXADD(___temp,y) \
 :___temp)
...
#ifdef ___USE_builtin_popcount

#if ___WORD_WIDTH == ___INT_WIDTH
#define ___POPCOUNT_WORD_TEMP() __builtin_popcount(___CAST(___UWORD,___temp))
#else
#if ___WORD_WIDTH == ___LONG_WIDTH
#define ___POPCOUNT_WORD_TEMP() __builtin_popcountl(___CAST(___UWORD,___temp))
#else
#define ___POPCOUNT_WORD_TEMP() __builtin_popcountll(___CAST(___UWORD,___temp))
#endif
#endif

#else

#define ___MASK_WORD_BYTEWISE(m) (~___CAST(___UWORD,0)/0xff*m)

#define ___POPCOUNT_WORD_TEMP() \
(___temp-=(___temp>>1)&___MASK_WORD_BYTEWISE(0x55),  \
 ___temp=(___temp&___MASK_WORD_BYTEWISE(0x33))+ \
         ((___temp>>2)&___MASK_WORD_BYTEWISE(0x33)), \
 (___MASK_WORD_BYTEWISE(0x01)* \
  (___MASK_WORD_BYTEWISE(0x0f)&(___temp+(___temp>>4)))) >> (___WORD_WIDTH-8))

#endif

#ifdef ___USE_builtin_ffs

#if ___WORD_WIDTH == ___INT_WIDTH
#define ___FFS_WORD_TEMP() __builtin_ffs(___temp)
#else
#if ___WORD_WIDTH == ___LONG_WIDTH
#define ___FFS_WORD_TEMP() __builtin_ffsl(___temp)
#else
#define ___FFS_WORD_TEMP() __builtin_ffsll(___temp)
#endif
#endif

#else

#define ___FFS_WORD_TEMP() \
(___temp==0 ? 0 : (___temp=((___temp-1)&~___temp),___POPCOUNT_WORD_TEMP()+1))

#endif

#ifdef ___USE_builtin_clrsb

#define ___LOG2_WORD_TEMP() ((___WORD_WIDTH-1) - ___CLRSB_WORD_TEMP())

#if ___WORD_WIDTH == ___INT_WIDTH
#define ___CLRSB_WORD_TEMP() __builtin_clrsb(___temp)
#else
#if ___WORD_WIDTH == ___LONG_WIDTH
#define ___CLRSB_WORD_TEMP() __builtin_clrsbl(___temp)
#else
#define ___CLRSB_WORD_TEMP() __builtin_clrsbll(___temp)
#endif
#endif

#else

#define ___SET_ALL_LOWER_BITS_WORD_TEMP_32() \
(___temp|=(___temp>>1), \
 ___temp|=(___temp>>2), \
 ___temp|=(___temp>>4), \
 ___temp|=(___temp>>8), \
...
#if ___WORD_WIDTH == 32
#define ___LOG2_WORD_TEMP() \
(___SET_ALL_LOWER_BITS_WORD_TEMP_32(), \
 ___POPCOUNT_WORD_TEMP())
#else
#define ___LOG2_WORD_TEMP() \
(___SET_ALL_LOWER_BITS_WORD_TEMP_32(), \
 ___temp|=(___temp>>32), \
...
(((___temp=___INT(x))<0)&&(___temp=~___temp),___FIX(___LOG2_WORD_TEMP()))

#define ___FIXBITCOUNT(x) \
(((___temp=___INT(x))<0)&&(___temp=~___temp),___FIX(___POPCOUNT_WORD_TEMP()))

#define ___FIXFIRSTSETBIT(x) \
(___temp=___INT(x),___FIX(___FFS_WORD_TEMP()-1))
...
#ifndef ___GET_CPUCYCLECOUNT
#ifdef ___USE___builtin_readcyclecounter

#if ___WORD_WIDTH == 64

#define ___GET_CPUCYCLECOUNT ___FIX(___s64_temp)
#define ___CPUCYCLECOUNTSTART ___s64_temp = __builtin_readcyclecounter ();
#define ___CPUCYCLECOUNTEND ___s64_temp = __builtin_readcyclecounter ();

#else

#define ___GET_CPUCYCLECOUNT ___FIX(___s32_temp)
#define ___CPUCYCLECOUNTSTART ___s32_temp = __builtin_readcyclecounter ();
#define ___CPUCYCLECOUNTEND ___s32_temp = __builtin_readcyclecounter ();
...
#define ___GET_CPUCYCLECOUNT ___FIX(0)
#define ___CPUCYCLECOUNTSTART
#define ___CPUCYCLECOUNTEND
  ...
  if (___PRIMITIVETRYLOCK(mutex,___FIX(___OBJ_LOCK1),___FIX(___OBJ_LOCK2))) \
    { \
      ...
      if (___FIXGE(rlo,rhi)) \
       ...
       { \
         ___temp = ___STRINGREF(___FIELD(port,___PORT_CHAR_RBUF),rlo); \
         ___FIELD(port,___PORT_CHAR_RLO) = rlo = ___FIXADD(rlo,___FIX(1L)); \
         if (___CHAREQP(___temp,___CHR(10))) \
           { \
             ___FIELD(port,___PORT_CHAR_RCURLINE) = \
               ___FIXADD(___FIELD(port,___PORT_CHAR_RCHARS),rlo); \
             ___FIELD(port,___PORT_CHAR_RLINES) = \
               ___FIXADD(___FIELD(port,___PORT_CHAR_RLINES),___FIX(1L)); \
           } \
       } \
      ___PRIMITIVEUNLOCK(mutex,___FIX(___OBJ_LOCK1),___FIX(___OBJ_LOCK2)) \
    } \
...
if (___FIXLT(___FIXAND(___FIELD(port,___PORT_WOPTIONS),___FIX(2047L)),___FIX(1536L)) && (___FIXLT(___FIXAND(___FIELD(port,___PORT_WOPTIONS),___FIX(2047L)),___FIX(1024L)) || ___CHAREQP(c,___CHR(10)))) \
  ___temp = ___FAL; \
else \
  { \
    ___SCMOBJ wbuf; \
      ...
      { \
        ...
        if (___FIXGE(whi,___STRINGLENGTH(wbuf))) \
          ...
          { \
            ...
            if (___CHAREQP(c,___CHR(10))) \
              { \
                ___FIELD(port,___PORT_CHAR_WCURLINE) = \
                  ___FIXADD(___FIELD(port,___PORT_CHAR_WCHARS),whi); \
                ___FIELD(port,___PORT_CHAR_WLINES) = \
                  ___FIXADD(___FIELD(port,___PORT_CHAR_WLINES),___FIX(1L)); \
              } \
            ___temp = ___VOID; \
          } \
        ___PRIMITIVEUNLOCK(mutex,___FIX(___OBJ_LOCK1),___FIX(___OBJ_LOCK2)) \
...
#define ___CHARINPUTPORTPCACHED(x)(___STRUCTUREP(x)&&___STRUCTURETYPE(x)==___ps->type_cache[___CHAR_INPUT_PORT_TYPE_CACHE])
#define ___CHAROUTPUTPORTPCACHED(x)(___STRUCTUREP(x)&&___STRUCTURETYPE(x)==___ps->type_cache[___CHAR_OUTPUT_PORT_TYPE_CACHE])
...
#define ___GET_SMALL_ALLOC ___temp

#define ___MAKEVECTSMALL(size,fill,BEGIN_ALLOC_VECT,END_ALLOC_VECT,ADD_VECT_ELEM_RAW,GET_VECT,ELEM_TYPE) \
{ \
  int ___size = size; \
  ELEM_TYPE ___fill = fill; \
  int ___i; \
  BEGIN_ALLOC_VECT(___size) \
  for (___i=0; ___i<___size; ___i++) ADD_VECT_ELEM_RAW(___i,___fill) \
  END_ALLOC_VECT(___size) \
  ___temp = GET_VECT(___size); \
}

#define ___MAKEVECTSMALLNOFILL(size,BEGIN_ALLOC_VECT,END_ALLOC_VECT,ADD_VECT_ELEM_RAW,GET_VECT) \
{ \
  int ___size = size; \
  BEGIN_ALLOC_VECT(___size) \
  END_ALLOC_VECT(___size) \
...
#define ___SUBVECTSMALL(body,start,end,BEGIN_ALLOC_VECT,END_ALLOC_VECT,ADD_VECT_ELEM_RAW,GET_VECT,FETCH_RAW) \
{ \
  ___WORD *___body = body; \
  int ___i = start; \
  int ___size = (end)-___i; \
  int ___j; \
  BEGIN_ALLOC_VECT(___size) \
  for (___j=0; ___j<___size; ___j++) \
    { \
      ADD_VECT_ELEM_RAW(___j,FETCH_RAW(___body,___i)) \
      ___i++; \
    } \
  END_ALLOC_VECT(___size) \
  ___temp = GET_VECT(___size); \
...
#define ___VECTDELETESMALL(body,i,size,BEGIN_ALLOC_VECT,END_ALLOC_VECT,ADD_VECT_ELEM_RAW,GET_VECT,FETCH_RAW) \
{ \
  ___WORD *___body = body; \
  int ___i = i; \
  int ___size = (size)-1; \
  int ___j; \
  BEGIN_ALLOC_VECT(___size) \
  for (___j=0; ___j<___i; ___j++) \
    ADD_VECT_ELEM_RAW(___j,FETCH_RAW(___body,___j)) \
  for (; ___j<___size; ___j++) \
    ADD_VECT_ELEM_RAW(___j,FETCH_RAW(___body,___j+1)) \
  END_ALLOC_VECT(___size) \
  ___temp = GET_VECT(___size); \
}

#define ___VECTINSERTSMALL(body,i,val,size,BEGIN_ALLOC_VECT,END_ALLOC_VECT,ADD_VECT_ELEM_RAW,GET_VECT,FETCH_RAW) \
{ \
  ___WORD *___body = body; \
  int ___i = i; \
  int ___size = (size)+1; \
...
#define ___VECTUPDATESMALL(body,i,val,size,BEGIN_ALLOC_VECT,END_ALLOC_VECT,ADD_VECT_ELEM_RAW,GET_VECT,FETCH_RAW) \
{ \
  ___WORD *___body = body; \
  int ___size = size; \
  int ___j; \
...
#define ___VALUESSIZE(v)___HD_FIELDS(___SUBTYPED_HEADER(v))
#define ___VALUESLENGTH(v)___FIX(___VALUESSIZE(v))
...
#define ___MAKEVALUESSMALL1(size)___MAKEVALUESSMALL2(size,___FIX(0))
#define ___MAKEVALUESSMALL2(size,fill) \
___MAKEVECTSMALL(___INT(size),fill,___BEGIN_ALLOC_VALUES,___END_ALLOC_VALUES,___ADD_VALUES_ELEM,___GET_VALUES,___SCMOBJ)
#define ___VALUESCOPYSMALL1(v) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),0,___VALUESSIZE(v),___BEGIN_ALLOC_VALUES,___END_ALLOC_VALUES,___ADD_VALUES_ELEM,___GET_VALUES,___FETCH_WORD)
#define ___VALUESCOPYSMALL2(v,start) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___VALUESSIZE(v),___BEGIN_ALLOC_VALUES,___END_ALLOC_VALUES,___ADD_VALUES_ELEM,___GET_VALUES,___FETCH_WORD)
#define ___VALUESCOPYSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_VALUES,___END_ALLOC_VALUES,___ADD_VALUES_ELEM,___GET_VALUES,___FETCH_WORD)
#define ___SUBVALUESSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_VALUES,___END_ALLOC_VALUES,___ADD_VALUES_ELEM,___GET_VALUES,___FETCH_WORD)
#define ___VALUESDELETESMALL2(v,i) \
___VECTDELETESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___VALUESSIZE(v),___BEGIN_ALLOC_VALUES,___END_ALLOC_VALUES,___ADD_VALUES_ELEM,___GET_VALUES,___FETCH_WORD)
#define ___VALUESINSERTSMALL3(v,i,val) \
___VECTINSERTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),val,___VALUESSIZE(v),___BEGIN_ALLOC_VALUES,___END_ALLOC_VALUES,___ADD_VALUES_ELEM,___GET_VALUES,___FETCH_WORD)
#define ___VALUESUPDATESMALL3(v,i,val) \
___VECTUPDATESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),val,___VALUESSIZE(v),___BEGIN_ALLOC_VALUES,___END_ALLOC_VALUES,___ADD_VALUES_ELEM,___GET_VALUES,___FETCH_WORD)

#define ___VECTORSIZE(v)___HD_FIELDS(___SUBTYPED_HEADER(v))
#define ___VECTORLENGTH(v)___FIX(___VECTORSIZE(v))
...
#define ___MAKEVECTORSMALL1(size)___MAKEVECTORSMALL2(size,___FIX(0))
#define ___MAKEVECTORSMALL2(size,fill) \
___MAKEVECTSMALL(___INT(size),fill,___BEGIN_ALLOC_VECTOR,___END_ALLOC_VECTOR,___ADD_VECTOR_ELEM,___GET_VECTOR,___SCMOBJ)
#define ___VECTORCOPYSMALL1(v) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),0,___VECTORSIZE(v),___BEGIN_ALLOC_VECTOR,___END_ALLOC_VECTOR,___ADD_VECTOR_ELEM,___GET_VECTOR,___FETCH_WORD)
#define ___VECTORCOPYSMALL2(v,start) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___VECTORSIZE(v),___BEGIN_ALLOC_VECTOR,___END_ALLOC_VECTOR,___ADD_VECTOR_ELEM,___GET_VECTOR,___FETCH_WORD)
#define ___VECTORCOPYSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_VECTOR,___END_ALLOC_VECTOR,___ADD_VECTOR_ELEM,___GET_VECTOR,___FETCH_WORD)
#define ___SUBVECTORSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_VECTOR,___END_ALLOC_VECTOR,___ADD_VECTOR_ELEM,___GET_VECTOR,___FETCH_WORD)
#define ___VECTORDELETESMALL2(v,i) \
___VECTDELETESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___VECTORSIZE(v),___BEGIN_ALLOC_VECTOR,___END_ALLOC_VECTOR,___ADD_VECTOR_ELEM,___GET_VECTOR,___FETCH_WORD)
#define ___VECTORINSERTSMALL3(v,i,val) \
___VECTINSERTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),val,___VECTORSIZE(v),___BEGIN_ALLOC_VECTOR,___END_ALLOC_VECTOR,___ADD_VECTOR_ELEM,___GET_VECTOR,___FETCH_WORD)
#define ___VECTORUPDATESMALL3(v,i,val) \
___VECTUPDATESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),val,___VECTORSIZE(v),___BEGIN_ALLOC_VECTOR,___END_ALLOC_VECTOR,___ADD_VECTOR_ELEM,___GET_VECTOR,___FETCH_WORD)

#define ___STRINGSIZE(v)(___HD_BYTES(___SUBTYPED_HEADER(v))>>___LCS)
#define ___STRINGLENGTH(v)___FIX(___STRINGSIZE(v))
#define ___STRINGREF(v,i) \
___CHR(___FETCH_C(___BODY_AS(v,___tSUBTYPED),(i)>>___TB))
#define ___STRINGSET(v,i,val) \
___STORE_C(___BODY_AS(v,___tSUBTYPED),(i)>>___TB,___INT(val));
#define ___STRINGSHRINK(v,i)___temp=v;___SUBTYPED_HEADER_SET(___temp,(___SUBTYPED_HEADER(___temp)&~___LMASK)+((i)<<(___LF-___TB+___LCS)));

#define ___MAKESTRINGSMALL1(size) \
___MAKEVECTSMALLNOFILL(___INT(size),___BEGIN_ALLOC_STRING,___END_ALLOC_STRING,___ADD_STRING_ELEM_RAW,___GET_STRING)
#define ___MAKESTRINGSMALL2(size,fill) \
___MAKEVECTSMALL(___INT(size),___INT(fill),___BEGIN_ALLOC_STRING,___END_ALLOC_STRING,___ADD_STRING_ELEM_RAW,___GET_STRING,___C)
#define ___STRINGCOPYSMALL1(v) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),0,___STRINGSIZE(v),___BEGIN_ALLOC_STRING,___END_ALLOC_STRING,___ADD_STRING_ELEM_RAW,___GET_STRING,___FETCH_C)
#define ___STRINGCOPYSMALL2(v,start) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___STRINGSIZE(v),___BEGIN_ALLOC_STRING,___END_ALLOC_STRING,___ADD_STRING_ELEM_RAW,___GET_STRING,___FETCH_C)
#define ___STRINGCOPYSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_STRING,___END_ALLOC_STRING,___ADD_STRING_ELEM_RAW,___GET_STRING,___FETCH_C)
#define ___SUBSTRINGSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_STRING,___END_ALLOC_STRING,___ADD_STRING_ELEM_RAW,___GET_STRING,___FETCH_C)
#define ___STRINGDELETESMALL2(v,i) \
___VECTDELETESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___STRINGSIZE(v),___BEGIN_ALLOC_STRING,___END_ALLOC_STRING,___ADD_STRING_ELEM_RAW,___GET_STRING,___FETCH_C)
#define ___STRINGINSERTSMALL3(v,i,val) \
___VECTINSERTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___INT(val),___STRINGSIZE(v),___BEGIN_ALLOC_STRING,___END_ALLOC_STRING,___ADD_STRING_ELEM_RAW,___GET_STRING,___FETCH_C)
#define ___STRINGUPDATESMALL3(v,i,val) \
___VECTUPDATESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___INT(val),___STRINGSIZE(v),___BEGIN_ALLOC_STRING,___END_ALLOC_STRING,___ADD_STRING_ELEM_RAW,___GET_STRING,___FETCH_C)

#define ___S8VECTORSIZE(v)___HD_BYTES(___SUBTYPED_HEADER(v))
#define ___S8VECTORLENGTH(v)___FIX(___S8VECTORSIZE(v))
#define ___S8VECTORREF(v,i) \
___FIX(___FETCH_S8(___BODY_AS(v,___tSUBTYPED),(i)>>___TB))
#define ___S8VECTORSET(v,i,val) \
___STORE_S8(___BODY_AS(v,___tSUBTYPED),(i)>>___TB,___INT(val));
#define ___S8VECTORSHRINK(v,i)___temp=v;___SUBTYPED_HEADER_SET(___temp,(___SUBTYPED_HEADER(___temp)&~___LMASK)+((i)<<(___LF-___TB)));

#define ___MAKES8VECTORSMALL1(size) \
___MAKEVECTSMALLNOFILL(___INT(size),___BEGIN_ALLOC_S8VECTOR,___END_ALLOC_S8VECTOR,___ADD_S8VECTOR_ELEM_RAW,___GET_S8VECTOR)
#define ___MAKES8VECTORSMALL2(size,fill) \
___MAKEVECTSMALL(___INT(size),___INT(fill),___BEGIN_ALLOC_S8VECTOR,___END_ALLOC_S8VECTOR,___ADD_S8VECTOR_ELEM_RAW,___GET_S8VECTOR,___S8)
#define ___S8VECTORCOPYSMALL1(v) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),0,___S8VECTORSIZE(v),___BEGIN_ALLOC_S8VECTOR,___END_ALLOC_S8VECTOR,___ADD_S8VECTOR_ELEM_RAW,___GET_S8VECTOR,___FETCH_S8)
#define ___S8VECTORCOPYSMALL2(v,start) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___S8VECTORSIZE(v),___BEGIN_ALLOC_S8VECTOR,___END_ALLOC_S8VECTOR,___ADD_S8VECTOR_ELEM_RAW,___GET_S8VECTOR,___FETCH_S8)
#define ___S8VECTORCOPYSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_S8VECTOR,___END_ALLOC_S8VECTOR,___ADD_S8VECTOR_ELEM_RAW,___GET_S8VECTOR,___FETCH_S8)
#define ___SUBS8VECTORSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_S8VECTOR,___END_ALLOC_S8VECTOR,___ADD_S8VECTOR_ELEM_RAW,___GET_S8VECTOR,___FETCH_S8)
#define ___S8VECTORDELETESMALL2(v,i) \
___VECTDELETESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___S8VECTORSIZE(v),___BEGIN_ALLOC_S8VECTOR,___END_ALLOC_S8VECTOR,___ADD_S8VECTOR_ELEM_RAW,___GET_S8VECTOR,___FETCH_S8)
#define ___S8VECTORINSERTSMALL3(v,i,val) \
___VECTINSERTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___INT(val),___S8VECTORSIZE(v),___BEGIN_ALLOC_S8VECTOR,___END_ALLOC_S8VECTOR,___ADD_S8VECTOR_ELEM_RAW,___GET_S8VECTOR,___FETCH_S8)
#define ___S8VECTORUPDATESMALL3(v,i,val) \
___VECTUPDATESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___INT(val),___S8VECTORSIZE(v),___BEGIN_ALLOC_S8VECTOR,___END_ALLOC_S8VECTOR,___ADD_S8VECTOR_ELEM_RAW,___GET_S8VECTOR,___FETCH_S8)

#define ___U8VECTORSIZE(v)___HD_BYTES(___SUBTYPED_HEADER(v))
#define ___U8VECTORLENGTH(v)___FIX(___U8VECTORSIZE(v))
#define ___U8VECTORREF(v,i) \
___FIX(___FETCH_U8(___BODY_AS(v,___tSUBTYPED),(i)>>___TB))
#define ___U8VECTORSET(v,i,val) \
___STORE_U8(___BODY_AS(v,___tSUBTYPED),(i)>>___TB,___INT(val));
#define ___U8VECTORSHRINK(v,i)___temp=v;___SUBTYPED_HEADER_SET(___temp,(___SUBTYPED_HEADER(___temp)&~___LMASK)+((i)<<(___LF-___TB)));

#define ___MAKEU8VECTORSMALL1(size) \
___MAKEVECTSMALLNOFILL(___INT(size),___BEGIN_ALLOC_U8VECTOR,___END_ALLOC_U8VECTOR,___ADD_U8VECTOR_ELEM_RAW,___GET_U8VECTOR)
#define ___MAKEU8VECTORSMALL2(size,fill) \
___MAKEVECTSMALL(___INT(size),___INT(fill),___BEGIN_ALLOC_U8VECTOR,___END_ALLOC_U8VECTOR,___ADD_U8VECTOR_ELEM_RAW,___GET_U8VECTOR,___U8)
#define ___U8VECTORCOPYSMALL1(v) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),0,___U8VECTORSIZE(v),___BEGIN_ALLOC_U8VECTOR,___END_ALLOC_U8VECTOR,___ADD_U8VECTOR_ELEM_RAW,___GET_U8VECTOR,___FETCH_U8)
#define ___U8VECTORCOPYSMALL2(v,start) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___U8VECTORSIZE(v),___BEGIN_ALLOC_U8VECTOR,___END_ALLOC_U8VECTOR,___ADD_U8VECTOR_ELEM_RAW,___GET_U8VECTOR,___FETCH_U8)
#define ___U8VECTORCOPYSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_U8VECTOR,___END_ALLOC_U8VECTOR,___ADD_U8VECTOR_ELEM_RAW,___GET_U8VECTOR,___FETCH_U8)
#define ___SUBU8VECTORSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_U8VECTOR,___END_ALLOC_U8VECTOR,___ADD_U8VECTOR_ELEM_RAW,___GET_U8VECTOR,___FETCH_U8)
#define ___U8VECTORDELETESMALL2(v,i) \
___VECTDELETESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___U8VECTORSIZE(v),___BEGIN_ALLOC_U8VECTOR,___END_ALLOC_U8VECTOR,___ADD_U8VECTOR_ELEM_RAW,___GET_U8VECTOR,___FETCH_U8)
#define ___U8VECTORINSERTSMALL3(v,i,val) \
___VECTINSERTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___INT(val),___U8VECTORSIZE(v),___BEGIN_ALLOC_U8VECTOR,___END_ALLOC_U8VECTOR,___ADD_U8VECTOR_ELEM_RAW,___GET_U8VECTOR,___FETCH_U8)
#define ___U8VECTORUPDATESMALL3(v,i,val) \
___VECTUPDATESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___INT(val),___U8VECTORSIZE(v),___BEGIN_ALLOC_U8VECTOR,___END_ALLOC_U8VECTOR,___ADD_U8VECTOR_ELEM_RAW,___GET_U8VECTOR,___FETCH_U8)

#define ___S16VECTORSIZE(v)(___HD_BYTES(___SUBTYPED_HEADER(v))>>1)
#define ___S16VECTORLENGTH(v)___FIX(___S16VECTORSIZE(v))
#define ___S16VECTORREF(v,i) \
___FIX(___FETCH_S16(___BODY_AS(v,___tSUBTYPED),(i)>>___TB))
#define ___S16VECTORSET(v,i,val) \
___STORE_S16(___BODY_AS(v,___tSUBTYPED),(i)>>___TB,___INT(val));
#define ___S16VECTORSHRINK(v,i)___temp=v;___SUBTYPED_HEADER_SET(___temp,(___SUBTYPED_HEADER(___temp)&~___LMASK)+((i)<<(___LF-___TB+1)));

#define ___MAKES16VECTORSMALL1(size) \
___MAKEVECTSMALLNOFILL(___INT(size),___BEGIN_ALLOC_S16VECTOR,___END_ALLOC_S16VECTOR,___ADD_S16VECTOR_ELEM_RAW,___GET_S16VECTOR)
#define ___MAKES16VECTORSMALL2(size,fill) \
___MAKEVECTSMALL(___INT(size),___INT(fill),___BEGIN_ALLOC_S16VECTOR,___END_ALLOC_S16VECTOR,___ADD_S16VECTOR_ELEM_RAW,___GET_S16VECTOR,___S16)
#define ___S16VECTORCOPYSMALL1(v) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),0,___S16VECTORSIZE(v),___BEGIN_ALLOC_S16VECTOR,___END_ALLOC_S16VECTOR,___ADD_S16VECTOR_ELEM_RAW,___GET_S16VECTOR,___FETCH_S16)
#define ___S16VECTORCOPYSMALL2(v,start) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___S16VECTORSIZE(v),___BEGIN_ALLOC_S16VECTOR,___END_ALLOC_S16VECTOR,___ADD_S16VECTOR_ELEM_RAW,___GET_S16VECTOR,___FETCH_S16)
#define ___S16VECTORCOPYSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_S16VECTOR,___END_ALLOC_S16VECTOR,___ADD_S16VECTOR_ELEM_RAW,___GET_S16VECTOR,___FETCH_S16)
#define ___SUBS16VECTORSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_S16VECTOR,___END_ALLOC_S16VECTOR,___ADD_S16VECTOR_ELEM_RAW,___GET_S16VECTOR,___FETCH_S16)
#define ___S16VECTORDELETESMALL2(v,i) \
___VECTDELETESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___S16VECTORSIZE(v),___BEGIN_ALLOC_S16VECTOR,___END_ALLOC_S16VECTOR,___ADD_S16VECTOR_ELEM_RAW,___GET_S16VECTOR,___FETCH_S16)
#define ___S16VECTORINSERTSMALL3(v,i,val) \
___VECTINSERTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___INT(val),___S16VECTORSIZE(v),___BEGIN_ALLOC_S16VECTOR,___END_ALLOC_S16VECTOR,___ADD_S16VECTOR_ELEM_RAW,___GET_S16VECTOR,___FETCH_S16)
#define ___S16VECTORUPDATESMALL3(v,i,val) \
___VECTUPDATESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___INT(val),___S16VECTORSIZE(v),___BEGIN_ALLOC_S16VECTOR,___END_ALLOC_S16VECTOR,___ADD_S16VECTOR_ELEM_RAW,___GET_S16VECTOR,___FETCH_S16)

#define ___U16VECTORSIZE(v)(___HD_BYTES(___SUBTYPED_HEADER(v))>>1)
#define ___U16VECTORLENGTH(v)___FIX(___U16VECTORSIZE(v))
#define ___U16VECTORREF(v,i) \
___FIX(___FETCH_U16(___BODY_AS(v,___tSUBTYPED),(i)>>___TB))
#define ___U16VECTORSET(v,i,val) \
___STORE_U16(___BODY_AS(v,___tSUBTYPED),(i)>>___TB,___INT(val));
#define ___U16VECTORSHRINK(v,i)___temp=v;___SUBTYPED_HEADER_SET(___temp,(___SUBTYPED_HEADER(___temp)&~___LMASK)+((i)<<(___LF-___TB+1)));

#define ___MAKEU16VECTORSMALL1(size) \
___MAKEVECTSMALLNOFILL(___INT(size),___BEGIN_ALLOC_U16VECTOR,___END_ALLOC_U16VECTOR,___ADD_U16VECTOR_ELEM_RAW,___GET_U16VECTOR)
#define ___MAKEU16VECTORSMALL2(size,fill) \
___MAKEVECTSMALL(___INT(size),___INT(fill),___BEGIN_ALLOC_U16VECTOR,___END_ALLOC_U16VECTOR,___ADD_U16VECTOR_ELEM_RAW,___GET_U16VECTOR,___U16)
#define ___U16VECTORCOPYSMALL1(v) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),0,___U16VECTORSIZE(v),___BEGIN_ALLOC_U16VECTOR,___END_ALLOC_U16VECTOR,___ADD_U16VECTOR_ELEM_RAW,___GET_U16VECTOR,___FETCH_U16)
#define ___U16VECTORCOPYSMALL2(v,start) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___U16VECTORSIZE(v),___BEGIN_ALLOC_U16VECTOR,___END_ALLOC_U16VECTOR,___ADD_U16VECTOR_ELEM_RAW,___GET_U16VECTOR,___FETCH_U16)
#define ___U16VECTORCOPYSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_U16VECTOR,___END_ALLOC_U16VECTOR,___ADD_U16VECTOR_ELEM_RAW,___GET_U16VECTOR,___FETCH_U16)
#define ___SUBU16VECTORSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_U16VECTOR,___END_ALLOC_U16VECTOR,___ADD_U16VECTOR_ELEM_RAW,___GET_U16VECTOR,___FETCH_U16)
#define ___U16VECTORDELETESMALL2(v,i) \
___VECTDELETESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___U16VECTORSIZE(v),___BEGIN_ALLOC_U16VECTOR,___END_ALLOC_U16VECTOR,___ADD_U16VECTOR_ELEM_RAW,___GET_U16VECTOR,___FETCH_U16)
#define ___U16VECTORINSERTSMALL3(v,i,val) \
___VECTINSERTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___INT(val),___U16VECTORSIZE(v),___BEGIN_ALLOC_U16VECTOR,___END_ALLOC_U16VECTOR,___ADD_U16VECTOR_ELEM_RAW,___GET_U16VECTOR,___FETCH_U16)
#define ___U16VECTORUPDATESMALL3(v,i,val) \
___VECTUPDATESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___INT(val),___U16VECTORSIZE(v),___BEGIN_ALLOC_U16VECTOR,___END_ALLOC_U16VECTOR,___ADD_U16VECTOR_ELEM_RAW,___GET_U16VECTOR,___FETCH_U16)

#define ___S32VECTORSIZE(v)(___HD_BYTES(___SUBTYPED_HEADER(v))>>2)
#define ___S32VECTORLENGTH(v)___FIX(___S32VECTORSIZE(v))
#define ___S32VECTORREF(v,i) \
___S32BOX(___FETCH_S32(___BODY_AS(v,___tSUBTYPED),(i)>>___TB))
#define ___S32VECTORSET(v,i,val) \
___STORE_S32(___BODY_AS(v,___tSUBTYPED),(i)>>___TB,___S32UNBOX(val));
#define ___S32VECTORSHRINK(v,i)___temp=v;___SUBTYPED_HEADER_SET(___temp,(___SUBTYPED_HEADER(___temp)&~___LMASK)+((i)<<(___LF-___TB+2)));

#define ___MAKES32VECTORSMALL1(size) \
___MAKEVECTSMALLNOFILL(___INT(size),___BEGIN_ALLOC_S32VECTOR,___END_ALLOC_S32VECTOR,___ADD_S32VECTOR_ELEM_RAW,___GET_S32VECTOR)
#define ___MAKES32VECTORSMALL2(size,fill) \
___MAKEVECTSMALL(___INT(size),___S32UNBOX(fill),___BEGIN_ALLOC_S32VECTOR,___END_ALLOC_S32VECTOR,___ADD_S32VECTOR_ELEM_RAW,___GET_S32VECTOR,___S32)
#define ___S32VECTORCOPYSMALL1(v) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),0,___S32VECTORSIZE(v),___BEGIN_ALLOC_S32VECTOR,___END_ALLOC_S32VECTOR,___ADD_S32VECTOR_ELEM_RAW,___GET_S32VECTOR,___FETCH_S32)
#define ___S32VECTORCOPYSMALL2(v,start) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___S32VECTORSIZE(v),___BEGIN_ALLOC_S32VECTOR,___END_ALLOC_S32VECTOR,___ADD_S32VECTOR_ELEM_RAW,___GET_S32VECTOR,___FETCH_S32)
#define ___S32VECTORCOPYSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_S32VECTOR,___END_ALLOC_S32VECTOR,___ADD_S32VECTOR_ELEM_RAW,___GET_S32VECTOR,___FETCH_S32)
#define ___SUBS32VECTORSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_S32VECTOR,___END_ALLOC_S32VECTOR,___ADD_S32VECTOR_ELEM_RAW,___GET_S32VECTOR,___FETCH_S32)
#define ___S32VECTORDELETESMALL2(v,i) \
___VECTDELETESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___S32VECTORSIZE(v),___BEGIN_ALLOC_S32VECTOR,___END_ALLOC_S32VECTOR,___ADD_S32VECTOR_ELEM_RAW,___GET_S32VECTOR,___FETCH_S32)
#define ___S32VECTORINSERTSMALL3(v,i,val) \
___VECTINSERTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___S32UNBOX(val),___S32VECTORSIZE(v),___BEGIN_ALLOC_S32VECTOR,___END_ALLOC_S32VECTOR,___ADD_S32VECTOR_ELEM_RAW,___GET_S32VECTOR,___FETCH_S32)
#define ___S32VECTORUPDATESMALL3(v,i,val) \
___VECTUPDATESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___S32UNBOX(val),___S32VECTORSIZE(v),___BEGIN_ALLOC_S32VECTOR,___END_ALLOC_S32VECTOR,___ADD_S32VECTOR_ELEM_RAW,___GET_S32VECTOR,___FETCH_S32)

#define ___U32VECTORSIZE(v)(___HD_BYTES(___SUBTYPED_HEADER(v))>>2)
#define ___U32VECTORLENGTH(v)___FIX(___U32VECTORSIZE(v))
#define ___U32VECTORREF(v,i) \
___U32BOX(___FETCH_U32(___BODY_AS(v,___tSUBTYPED),(i)>>___TB))
#define ___U32VECTORSET(v,i,val) \
___STORE_U32(___BODY_AS(v,___tSUBTYPED),(i)>>___TB,___U32UNBOX(val));
#define ___U32VECTORSHRINK(v,i)___temp=v;___SUBTYPED_HEADER_SET(___temp,(___SUBTYPED_HEADER(___temp)&~___LMASK)+((i)<<(___LF-___TB+2)));

#define ___MAKEU32VECTORSMALL1(size) \
___MAKEVECTSMALLNOFILL(___INT(size),___BEGIN_ALLOC_U32VECTOR,___END_ALLOC_U32VECTOR,___ADD_U32VECTOR_ELEM_RAW,___GET_U32VECTOR)
#define ___MAKEU32VECTORSMALL2(size,fill) \
___MAKEVECTSMALL(___INT(size),___U32UNBOX(fill),___BEGIN_ALLOC_U32VECTOR,___END_ALLOC_U32VECTOR,___ADD_U32VECTOR_ELEM_RAW,___GET_U32VECTOR,___U32)
#define ___U32VECTORCOPYSMALL1(v) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),0,___U32VECTORSIZE(v),___BEGIN_ALLOC_U32VECTOR,___END_ALLOC_U32VECTOR,___ADD_U32VECTOR_ELEM_RAW,___GET_U32VECTOR,___FETCH_U32)
#define ___U32VECTORCOPYSMALL2(v,start) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___U32VECTORSIZE(v),___BEGIN_ALLOC_U32VECTOR,___END_ALLOC_U32VECTOR,___ADD_U32VECTOR_ELEM_RAW,___GET_U32VECTOR,___FETCH_U32)
#define ___U32VECTORCOPYSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_U32VECTOR,___END_ALLOC_U32VECTOR,___ADD_U32VECTOR_ELEM_RAW,___GET_U32VECTOR,___FETCH_U32)
#define ___SUBU32VECTORSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_U32VECTOR,___END_ALLOC_U32VECTOR,___ADD_U32VECTOR_ELEM_RAW,___GET_U32VECTOR,___FETCH_U32)
#define ___U32VECTORDELETESMALL2(v,i) \
___VECTDELETESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___U32VECTORSIZE(v),___BEGIN_ALLOC_U32VECTOR,___END_ALLOC_U32VECTOR,___ADD_U32VECTOR_ELEM_RAW,___GET_U32VECTOR,___FETCH_U32)
#define ___U32VECTORINSERTSMALL3(v,i,val) \
___VECTINSERTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___U32UNBOX(val),___U32VECTORSIZE(v),___BEGIN_ALLOC_U32VECTOR,___END_ALLOC_U32VECTOR,___ADD_U32VECTOR_ELEM_RAW,___GET_U32VECTOR,___FETCH_U32)
#define ___U32VECTORUPDATESMALL3(v,i,val) \
___VECTUPDATESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___U32UNBOX(val),___U32VECTORSIZE(v),___BEGIN_ALLOC_U32VECTOR,___END_ALLOC_U32VECTOR,___ADD_U32VECTOR_ELEM_RAW,___GET_U32VECTOR,___FETCH_U32)

#define ___S64VECTORSIZE(v)(___HD_BYTES(___SUBTYPED_HEADER(v))>>3)
#define ___S64VECTORLENGTH(v)___FIX(___S64VECTORSIZE(v))
#define ___S64VECTORREF(v,i) \
___S64BOX(___FETCH_S64(___BODY_AS(v,___tSUBTYPED),(i)>>___TB))
#define ___S64VECTORSET(v,i,val) \
___STORE_S64(___BODY_AS(v,___tSUBTYPED),(i)>>___TB,___S64UNBOX(val));
#define ___S64VECTORSHRINK(v,i)___temp=v;___SUBTYPED_HEADER_SET(___temp,(___SUBTYPED_HEADER(___temp)&~___LMASK)+((i)<<(___LF-___TB+3)));

#define ___MAKES64VECTORSMALL1(size) \
___MAKEVECTSMALLNOFILL(___INT(size),___BEGIN_ALLOC_S64VECTOR,___END_ALLOC_S64VECTOR,___ADD_S64VECTOR_ELEM_RAW,___GET_S64VECTOR)
#define ___MAKES64VECTORSMALL2(size,fill) \
___MAKEVECTSMALL(___INT(size),___S64UNBOX(fill),___BEGIN_ALLOC_S64VECTOR,___END_ALLOC_S64VECTOR,___ADD_S64VECTOR_ELEM_RAW,___GET_S64VECTOR,___S64)
#define ___S64VECTORCOPYSMALL1(v) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),0,___S64VECTORSIZE(v),___BEGIN_ALLOC_S64VECTOR,___END_ALLOC_S64VECTOR,___ADD_S64VECTOR_ELEM_RAW,___GET_S64VECTOR,___FETCH_S64)
#define ___S64VECTORCOPYSMALL2(v,start) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___S64VECTORSIZE(v),___BEGIN_ALLOC_S64VECTOR,___END_ALLOC_S64VECTOR,___ADD_S64VECTOR_ELEM_RAW,___GET_S64VECTOR,___FETCH_S64)
#define ___S64VECTORCOPYSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_S64VECTOR,___END_ALLOC_S64VECTOR,___ADD_S64VECTOR_ELEM_RAW,___GET_S64VECTOR,___FETCH_S64)
#define ___SUBS64VECTORSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_S64VECTOR,___END_ALLOC_S64VECTOR,___ADD_S64VECTOR_ELEM_RAW,___GET_S64VECTOR,___FETCH_S64)
#define ___S64VECTORDELETESMALL2(v,i) \
___VECTDELETESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___S64VECTORSIZE(v),___BEGIN_ALLOC_S64VECTOR,___END_ALLOC_S64VECTOR,___ADD_S64VECTOR_ELEM_RAW,___GET_S64VECTOR,___FETCH_S64)
#define ___S64VECTORINSERTSMALL3(v,i,val) \
___VECTINSERTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___S64UNBOX(val),___S64VECTORSIZE(v),___BEGIN_ALLOC_S64VECTOR,___END_ALLOC_S64VECTOR,___ADD_S64VECTOR_ELEM_RAW,___GET_S64VECTOR,___FETCH_S64)
#define ___S64VECTORUPDATESMALL3(v,i,val) \
___VECTUPDATESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___S64UNBOX(val),___S64VECTORSIZE(v),___BEGIN_ALLOC_S64VECTOR,___END_ALLOC_S64VECTOR,___ADD_S64VECTOR_ELEM_RAW,___GET_S64VECTOR,___FETCH_S64)

#define ___U64VECTORSIZE(v)(___HD_BYTES(___SUBTYPED_HEADER(v))>>3)
#define ___U64VECTORLENGTH(v)___FIX(___U64VECTORSIZE(v))
#define ___U64VECTORREF(v,i) \
___U64BOX(___FETCH_U64(___BODY_AS(v,___tSUBTYPED),(i)>>___TB))
#define ___U64VECTORSET(v,i,val) \
___STORE_U64(___BODY_AS(v,___tSUBTYPED),(i)>>___TB,___U64UNBOX(val));
#define ___U64VECTORSHRINK(v,i)___temp=v;___SUBTYPED_HEADER_SET(___temp,(___SUBTYPED_HEADER(___temp)&~___LMASK)+((i)<<(___LF-___TB+3)));

#define ___MAKEU64VECTORSMALL1(size) \
___MAKEVECTSMALLNOFILL(___INT(size),___BEGIN_ALLOC_U64VECTOR,___END_ALLOC_U64VECTOR,___ADD_U64VECTOR_ELEM_RAW,___GET_U64VECTOR)
#define ___MAKEU64VECTORSMALL2(size,fill) \
___MAKEVECTSMALL(___INT(size),___U64UNBOX(fill),___BEGIN_ALLOC_U64VECTOR,___END_ALLOC_U64VECTOR,___ADD_U64VECTOR_ELEM_RAW,___GET_U64VECTOR,___U64)
#define ___U64VECTORCOPYSMALL1(v) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),0,___U64VECTORSIZE(v),___BEGIN_ALLOC_U64VECTOR,___END_ALLOC_U64VECTOR,___ADD_U64VECTOR_ELEM_RAW,___GET_U64VECTOR,___FETCH_U64)
#define ___U64VECTORCOPYSMALL2(v,start) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___U64VECTORSIZE(v),___BEGIN_ALLOC_U64VECTOR,___END_ALLOC_U64VECTOR,___ADD_U64VECTOR_ELEM_RAW,___GET_U64VECTOR,___FETCH_U64)
#define ___U64VECTORCOPYSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_U64VECTOR,___END_ALLOC_U64VECTOR,___ADD_U64VECTOR_ELEM_RAW,___GET_U64VECTOR,___FETCH_U64)
#define ___SUBU64VECTORSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_U64VECTOR,___END_ALLOC_U64VECTOR,___ADD_U64VECTOR_ELEM_RAW,___GET_U64VECTOR,___FETCH_U64)
#define ___U64VECTORDELETESMALL2(v,i) \
___VECTDELETESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___U64VECTORSIZE(v),___BEGIN_ALLOC_U64VECTOR,___END_ALLOC_U64VECTOR,___ADD_U64VECTOR_ELEM_RAW,___GET_U64VECTOR,___FETCH_U64)
#define ___U64VECTORINSERTSMALL3(v,i,val) \
___VECTINSERTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___U64UNBOX(val),___U64VECTORSIZE(v),___BEGIN_ALLOC_U64VECTOR,___END_ALLOC_U64VECTOR,___ADD_U64VECTOR_ELEM_RAW,___GET_U64VECTOR,___FETCH_U64)
#define ___U64VECTORUPDATESMALL3(v,i,val) \
___VECTUPDATESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___U64UNBOX(val),___U64VECTORSIZE(v),___BEGIN_ALLOC_U64VECTOR,___END_ALLOC_U64VECTOR,___ADD_U64VECTOR_ELEM_RAW,___GET_U64VECTOR,___FETCH_U64)

#define ___F32VECTORSIZE(v)(___HD_BYTES(___SUBTYPED_HEADER(v))>>2)
#define ___F32VECTORLENGTH(v)___FIX(___F32VECTORSIZE(v))
#ifdef ___F32
#define ___F32VECTORREF(v,i)*(___F32*)(((___WORD)___BODY_AS(v,___tSUBTYPED))+((i)<<(2-___TB)))
#define ___F32VECTORSET(v,i,val)*(___F32*)(((___WORD)___BODY_AS(v,___tSUBTYPED))+((i)<<(2-___TB)))=val;
#else
#define ___F32VECTORREF(v,i)0.0
#define ___F32VECTORSET(v,i,val);
#endif
#define ___F32VECTORSHRINK(v,i)___temp=v;___SUBTYPED_HEADER_SET(___temp,(___SUBTYPED_HEADER(___temp)&~___LMASK)+((i)<<(___LF-___TB+2)));

#define ___MAKEF32VECTORSMALL1(size) \
___MAKEVECTSMALLNOFILL(___INT(size),___BEGIN_ALLOC_F32VECTOR,___END_ALLOC_F32VECTOR,___ADD_F32VECTOR_ELEM,___GET_F32VECTOR)
#define ___MAKEF32VECTORSMALL2(size,fill) \
___MAKEVECTSMALL(___INT(size),___F64UNBOX(fill),___BEGIN_ALLOC_F32VECTOR,___END_ALLOC_F32VECTOR,___ADD_F32VECTOR_ELEM,___GET_F32VECTOR,___F64)
#define ___F32VECTORCOPYSMALL1(v) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),0,___F32VECTORSIZE(v),___BEGIN_ALLOC_F32VECTOR,___END_ALLOC_F32VECTOR,___ADD_F32VECTOR_ELEM,___GET_F32VECTOR,___FETCH_F32)
#define ___F32VECTORCOPYSMALL2(v,start) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___F32VECTORSIZE(v),___BEGIN_ALLOC_F32VECTOR,___END_ALLOC_F32VECTOR,___ADD_F32VECTOR_ELEM,___GET_F32VECTOR,___FETCH_F32)
#define ___F32VECTORCOPYSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_F32VECTOR,___END_ALLOC_F32VECTOR,___ADD_F32VECTOR_ELEM,___GET_F32VECTOR,___FETCH_F32)
#define ___SUBF32VECTORSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_F32VECTOR,___END_ALLOC_F32VECTOR,___ADD_F32VECTOR_ELEM,___GET_F32VECTOR,___FETCH_F32)
#define ___F32VECTORDELETESMALL2(v,i) \
___VECTDELETESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___F32VECTORSIZE(v),___BEGIN_ALLOC_F32VECTOR,___END_ALLOC_F32VECTOR,___ADD_F32VECTOR_ELEM,___GET_F32VECTOR,___FETCH_F32)
#define ___F32VECTORINSERTSMALL3(v,i,val) \
___VECTINSERTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___F64UNBOX(val),___F32VECTORSIZE(v),___BEGIN_ALLOC_F32VECTOR,___END_ALLOC_F32VECTOR,___ADD_F32VECTOR_ELEM,___GET_F32VECTOR,___FETCH_F32)
#define ___F32VECTORUPDATESMALL3(v,i,val) \
___VECTUPDATESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___F64UNBOX(val),___F32VECTORSIZE(v),___BEGIN_ALLOC_F32VECTOR,___END_ALLOC_F32VECTOR,___ADD_F32VECTOR_ELEM,___GET_F32VECTOR,___FETCH_F32)

#define ___F64VECTORSIZE(v)(___HD_BYTES(___SUBTYPED_HEADER(v))>>3)
#define ___F64VECTORLENGTH(v)___FIX(___F64VECTORSIZE(v))
#define ___F64VECTORREF(v,i)*(___F64*)(((___WORD)___BODY_AS(v,___tSUBTYPED))+((i)<<(3-___TB)))
#define ___F64VECTORSET(v,i,val)*(___F64*)(((___WORD)___BODY_AS(v,___tSUBTYPED))+((i)<<(3-___TB)))=val;
#define ___F64VECTORSHRINK(v,i)___temp=v;___SUBTYPED_HEADER_SET(___temp,(___SUBTYPED_HEADER(___temp)&~___LMASK)+((i)<<(___LF-___TB+3)));

#define ___MAKEF64VECTORSMALL1(size) \
___MAKEVECTSMALLNOFILL(___INT(size),___BEGIN_ALLOC_F64VECTOR,___END_ALLOC_F64VECTOR,___ADD_F64VECTOR_ELEM,___GET_F64VECTOR)
#define ___MAKEF64VECTORSMALL2(size,fill) \
___MAKEVECTSMALL(___INT(size),___F64UNBOX(fill),___BEGIN_ALLOC_F64VECTOR,___END_ALLOC_F64VECTOR,___ADD_F64VECTOR_ELEM,___GET_F64VECTOR,___F64)
#define ___F64VECTORCOPYSMALL1(v) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),0,___F64VECTORSIZE(v),___BEGIN_ALLOC_F64VECTOR,___END_ALLOC_F64VECTOR,___ADD_F64VECTOR_ELEM,___GET_F64VECTOR,___FETCH_F64)
#define ___F64VECTORCOPYSMALL2(v,start) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___F64VECTORSIZE(v),___BEGIN_ALLOC_F64VECTOR,___END_ALLOC_F64VECTOR,___ADD_F64VECTOR_ELEM,___GET_F64VECTOR,___FETCH_F64)
#define ___F64VECTORCOPYSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_F64VECTOR,___END_ALLOC_F64VECTOR,___ADD_F64VECTOR_ELEM,___GET_F64VECTOR,___FETCH_F64)
#define ___SUBF64VECTORSMALL3(v,start,end) \
___SUBVECTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(start),___INT(end),___BEGIN_ALLOC_F64VECTOR,___END_ALLOC_F64VECTOR,___ADD_F64VECTOR_ELEM,___GET_F64VECTOR,___FETCH_F64)
#define ___F64VECTORDELETESMALL2(v,i) \
___VECTDELETESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___F64VECTORSIZE(v),___BEGIN_ALLOC_F64VECTOR,___END_ALLOC_F64VECTOR,___ADD_F64VECTOR_ELEM,___GET_F64VECTOR,___FETCH_F64)
#define ___F64VECTORINSERTSMALL3(v,i,val) \
___VECTINSERTSMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___F64UNBOX(val),___F64VECTORSIZE(v),___BEGIN_ALLOC_F64VECTOR,___END_ALLOC_F64VECTOR,___ADD_F64VECTOR_ELEM,___GET_F64VECTOR,___FETCH_F64)
#define ___F64VECTORUPDATESMALL3(v,i,val) \
___VECTUPDATESMALL(___BODY_AS(v,___tSUBTYPED),___INT(i),___F64UNBOX(val),___F64VECTORSIZE(v),___BEGIN_ALLOC_F64VECTOR,___END_ALLOC_F64VECTOR,___ADD_F64VECTOR_ELEM,___GET_F64VECTOR,___FETCH_F64)
...
#define ___SYMBOL2STRINGP_NOTFALSEP(var,x)(___temp=___SYMBOLNAME(x),___EXPECT_FALSE(___FIXNUMP(___temp))?0:(var=___temp,1))
#define ___SYMBOL2STRINGP(x)(___SYMBOL2STRINGP_NOTFALSEP(___temp,x)?___temp:___FAL)
...
#define ___KEYWORD2STRINGP_NOTFALSEP(var,x)(___temp=___KEYWORDNAME(x),___EXPECT_FALSE(___FIXNUMP(___temp))?0:(var=___temp,1))
#define ___KEYWORD2STRINGP(x)(___KEYWORD2STRINGP_NOTFALSEP(___temp,x)?___temp:___FAL)
...
#define ___THREAD_INTERRUPTS_HEAD 32
#define ___THREAD_INTERRUPTS_TAIL 33
#define ___THREAD_LAST_PROCESSOR  34

#define ___PROCESSOR_SIZE            22
#define ___PROCESSOR_CURRENT_THREAD  14
#define ___PROCESSOR_INTERRUPTS_HEAD 20
#define ___PROCESSOR_INTERRUPTS_TAIL 21
...
#ifndef ___USE_TAIL_CALL_TO_EXIT_HOST

#define ___EXIT_HOST() /* fallthrough to end of host */

#else

#ifdef ___USE_PROPER_TAIL_CALL_EXCLUSIVELY_TO_EXIT_HOST

#define ___EXIT_HOST() ___TAIL_CALL_VOID(___CALL_LABEL_HOST(___pc,___ps))

#else

#define ___USE_UNWIND_C_STACK_TO_TRAMPOLINE

#define ___EXIT_HOST() \
do { \
  ___IF_C_STACK_TRIPPED(___c_stack_ptr) \
    ___EXT(___unwind_c_stack_to_trampoline) (___PSPNC); \
  ___TAIL_CALL_VOID(___CALL_LABEL_HOST(___pc,___ps)); \
} while (0)

#define ___C_STACK_PTR_DECL(id) \
___WORD ___c_stack_ptr_temp; \
___WORD *id = &___c_stack_ptr_temp;

#define ___SETUP_C_STACK_TRIP(id) \
___C_STACK_PTR_DECL(id) \
___ps->c_stack_trip = id

#define ___IF_C_STACK_TRIPPED(id) \
___C_STACK_PTR_DECL(id) \
if (___CAST(___U32,id-___ps->c_stack_trip+___C_STACK_TRIP_DIST) >= 2*___C_STACK_TRIP_DIST)

#ifndef ___C_STACK_TRIP_DIST
#define ___C_STACK_TRIP_DIST 2048
#endif
...
#define ___CALL_LABEL_HOST(pc,ps) ___LABEL_HOST_GET(pc)(ps)
...
#define ___END_HLBL \
0}; \
___pc=___ps->pc; ___R_ALL ___start=___MLBL(___HOST_LBL0); \
if (___EXPECT_FALSE(___pc == ___FIX(0))) { \
  ___ps->pc = ___CAST(___WORD,___hlbltbl); \
  return; \
}
...
#define ___JUMP(dest)___JUMPFAST(dest)
...
#define ___JUMPFAST(dest) \
{___pc=dest; \
 if (___EXPECT_FALSE(___LABEL_HOST_GET(___pc) != ___HOST_PROC)) \
   goto ___jumpext; \
 goto *___LABEL_HOST_LABEL_GET(___pc); \
}
...
#define ___END_HLBL ___pc=___ps->pc; ___R_ALL ___start=___MLBL(___HOST_LBL0);
...
#define ___JUMP(dest){___pc=dest;goto ___jump;}
#define ___JUMPEXT(dest){___pc=dest;goto ___jumpext;}
#define ___JUMPFAST(dest)___JUMP(dest)
...
#ifdef ___DONT_INLINE_JUMPS
#define ___JUMPRET(val)___JUMP(val)
#else
#define ___JUMPRET(val)___JUMPFAST(val)
#endif
...
#define ___CLOSURESIZE(x)___HD_FIELDS(___SUBTYPED_HEADER(x))-___CLO_LOWLEVEL_TRAMPOLINE_SIZE
#define ___CLOSURELENGTH(x)___FIX(___CLOSURESIZE(x))
...
#define ___ADD_STRING_ELEM_RAW(i,val)___STORE_C(___BODY0_FROM_HP(),i,val);
...
#define ___ADD_S8VECTOR_ELEM_RAW(i,val)___STORE_S8(___BODY0_FROM_HP(),i,val);
#define ___ADD_S8VECTOR_ELEM(i,val)___ADD_S8VECTOR_ELEM_RAW(i,___INT(val))
...
#define ___ADD_U8VECTOR_ELEM_RAW(i,val)___STORE_U8(___BODY0_FROM_HP(),i,val);
#define ___ADD_U8VECTOR_ELEM(i,val)___ADD_U8VECTOR_ELEM_RAW(i,___INT(val))
...
#define ___ADD_S16VECTOR_ELEM_RAW(i,val)___STORE_S16(___BODY0_FROM_HP(),i,val);
#define ___ADD_S16VECTOR_ELEM(i,val)___ADD_S16VECTOR_ELEM_RAW(i,___INT(val))
...
#define ___ADD_U16VECTOR_ELEM_RAW(i,val)___STORE_U16(___BODY0_FROM_HP(),i,val);
#define ___ADD_U16VECTOR_ELEM(i,val)___ADD_U16VECTOR_ELEM_RAW(i,___INT(val))
...
#define ___ADD_S32VECTOR_ELEM_RAW(i,val)___STORE_S32(___BODY0_FROM_HP(),i,val);
#define ___ADD_S32VECTOR_ELEM(i,val)___ADD_S32VECTOR_ELEM_RAW(i,___S32UNBOX(val))
...
#define ___ADD_U32VECTOR_ELEM_RAW(i,val)___STORE_U32(___BODY0_FROM_HP(),i,val);
#define ___ADD_U32VECTOR_ELEM(i,val)___ADD_U32VECTOR_ELEM_RAW(i,___U32UNBOX(val))
...
#define ___ADD_S64VECTOR_ELEM_RAW(i,val)___STORE_S64(___ptr,i,val);
#define ___ADD_S64VECTOR_ELEM(i,val)___ADD_S64VECTOR_ELEM_RAW(i,___S64UNBOX(val))
...
#define ___ADD_U64VECTOR_ELEM_RAW(i,val)___STORE_U64(___ptr,i,val);
#define ___ADD_U64VECTOR_ELEM(i,val)___ADD_U64VECTOR_ELEM_RAW(i,___U64UNBOX(val))
...
#define ___ADD_S64VECTOR_ELEM_RAW(i,val)___STORE_S64(___BODY0_FROM_HP(),i,val);
#define ___ADD_S64VECTOR_ELEM(i,val)___ADD_S64VECTOR_ELEM_RAW(i,___S64UNBOX(val))
...
#define ___ADD_U64VECTOR_ELEM_RAW(i,val)___STORE_U64(___BODY0_FROM_HP(),i,val);
#define ___ADD_U64VECTOR_ELEM(i,val)___ADD_U64VECTOR_ELEM_RAW(i,___U64UNBOX(val))
...
#define ___INTERRUPT_AFTER_MASKING(flags) \
((flags) & ___ps->intr_enabled & ~___ps->intr_mask)

#define ___INTERRUPT_REQ(flags) \
(___INTERRUPT_AFTER_MASKING(flags) != ___FIX(0))

#define ___STACK_TRIP_ON_IF_INTERRUPT_REQ() \
if (___INTERRUPT_REQ(___INTERRUPT_FLAGS())) \
  ___STACK_TRIP_ON()

#define ___INTERRUPT_MASK_SET(mask) \
do { \
  ___ps->intr_mask = (mask); \
...
#define ___GET_REAL_PSTATE() ___CAST(___processor_state,___EXT(___get_tls_ptr) ())
#define ___SET_REAL_PSTATE(ps) ___set_tls_ptr(___CAST(void*,ps))

#else

/* Use thread local storage class */

#define ___GET_REAL_PSTATE() ___CAST(___processor_state,___tls_ptr)
#define ___SET_REAL_PSTATE(ps) ___tls_ptr = ___CAST(void*,ps)

#endif

#endif

#ifndef ___SUPPORT_PSTATE_BIND

#define ___GET_PSTATE() ___GET_REAL_PSTATE()

#else

#define ___GET_PSTATE() ___GET_REAL_PSTATE()->pstate_binding
#define ___BEGIN_PSTATE_BIND(ps) \
do { \
  ___processor_state ___old_pstate = ___GET_REAL_PSTATE()->pstate_binding; \
  ___GET_REAL_PSTATE()->pstate_binding = ps
#define ___END_PSTATE_BIND() \
  ___GET_REAL_PSTATE()->pstate_binding = ___old_pstate; \
} while (0)
...
/* Continuation frame structures. */

#define ___IFD_FS_WIDTH 5
#define ___OFD_FS_WIDTH 14
#define ___KIND_WIDTH 2
...
#define ___DEF_OFD(kind,fs,link)(link<<16)+(fs<<___KIND_WIDTH)+kind
#define ___OFD_KIND(x)((x)&((1<<___KIND_WIDTH)-1))
#define ___OFD_FS(x)(((x)>>___KIND_WIDTH)&((1<<___OFD_FS_WIDTH)-1))
#define ___OFD_LINK(x)(((x)>>(___OFD_FS_WIDTH+___KIND_WIDTH))&((1<<___OFD_FS_WIDTH)-1))
...
#define ___IFD(kind,fs,link,gcmap)(gcmap<<(2*___IFD_FS_WIDTH+___KIND_WIDTH))+(link<<(___IFD_FS_WIDTH+___KIND_WIDTH))+(fs<<___KIND_WIDTH)+kind
#define ___OFD(kind,fs,link,gcmap)___IFD(kind,0,0,0)
#define ___IS_IFD(x)((x)&((1<<___KIND_WIDTH)-1))

#define ___IFD_KIND(x)((x)&((1<<___KIND_WIDTH)-1))
#define ___IFD_FS(x)(((x)>>___KIND_WIDTH)&((1<<___IFD_FS_WIDTH)-1))
#define ___IFD_LINK(x)(((x)>>(___IFD_FS_WIDTH+___KIND_WIDTH))&((1<<___IFD_FS_WIDTH)-1))
#define ___IFD_GCMAP(x)(((x)>>(2*___IFD_FS_WIDTH+___KIND_WIDTH))&((1<<(32-(2*___IFD_FS_WIDTH+___KIND_WIDTH)))-1))
...
/* Procedure descriptor structures. */

#define ___NBPARMS_WIDTH 12

#define ___PRD(nbparms,nbclosed) \
___MAKE_HD(((nbclosed+1)<<___NBPARMS_WIDTH)+nbparms,___sPROCEDURE,___PERM)
#define ___PRD_NBPARMS(x)(((x)>>(___SB+___HTB))&((1<<___NBPARMS_WIDTH)-1))
#define ___PRD_NBCLOSED(x)((((x)>>(___SB+___HTB+___NBPARMS_WIDTH))&((1<<___NBPARMS_WIDTH)-1))-1)
...
#define ___BEGIN_MOD_SYM_KEY ___LOCAL ___module_struct ___module_descr = { \
  ___VERSION, \
  ___MODULE_KIND, \
  sizeof(___module_struct), \
  sizeof(___global_state_struct), \
  sizeof(___virtual_machine_state_struct), \
  sizeof(___processor_state_struct), \
  ___MODULE_NAME, \
  ___MODULE_FLAGS, \
  ___GLOTBL, \
  ___SYMTBL, \
...
#define ___MODULE_INCOMPATIBILITY_ERR           (___ERR_BASE+10)
#define ___MODULE_ALREADY_LOADED_ERR            (___ERR_BASE+11)
#define ___DYNAMIC_LOADING_NOT_AVAILABLE_ERR    (___ERR_BASE+12)
#define ___DYNAMIC_LOADING_LOOKUP_ERR           (___ERR_BASE+13)
#define ___KILL_PUMP                            (___ERR_BASE+14)
#define ___SELECT_SETUP_DONE                    (___ERR_BASE+15)
...
#define ___UNWIND_C_STACK_TO_TRAMPOLINE         (___ERR_BASE+(126<<7))
#define ___CTOS_MAX                             (___ERR_BASE+(127<<7)-1)
#else
...
if (___err!=0) ___err = ___ps->longjmp_err; \
if (___err==___FIX(___NO_ERR)) {
#define ___END_TRY } ___ps->catcher = ___old_catcher; }
#define ___THROW(e)do { ___ps->longjmp_err = e; ___LONGJMP1(___ps->catcher->buf); } while (0)
...
#define ___BEGIN_SFUN_VOID(proc) ___processor_state ___ps=___PSTATE;___SCMOBJ ___marker;___SCMOBJ ___err;___SCMOBJ ___errdata=___FAL;___SCMOBJ ___errhandler=___FAL;if ((___err=___EXT(___make_sfun_stack_marker) (___ps,&___marker,proc))==___FIX(___NO_ERR)) {
...
#define ___SFUN_ERROR_VOID while ((___err!=___FIX(___NO_ERR))&&(___err!=___FIX(___UNWIND_C_STACK))) {___ps->temp1=___err;___ps->temp2=___errdata;___ps->temp3=___marker;___ps->temp4=___errhandler;___err=___EXT(___call) (___PSP 0,___ps->handler_sfun_conv_error,___marker);
#define ___END_SFUN_VOID } ___EXT(___kill_sfun_stack_marker) (___marker); } ___EXT(___propagate_error) (___PSP ___err);

#define ___BEGIN_SFUN_SCMOBJ(proc) ___processor_state ___ps=___PSTATE;___SCMOBJ ___marker;___SCMOBJ ___result;___SCMOBJ ___err;___SCMOBJ ___errdata=___FAL;___SCMOBJ ___errhandler=___FAL;if ((___err=___EXT(___make_sfun_stack_marker) (___ps,&___marker,proc))==___FIX(___NO_ERR)) {
...
#define ___SFUN_ERROR_SCMOBJ while ((___err!=___FIX(___NO_ERR))&&(___err!=___FIX(___UNWIND_C_STACK))) {___ps->temp1=___err;___ps->temp2=___errdata;___ps->temp3=___marker;___ps->temp4=___errhandler;___err=___EXT(___call) (___PSP 0,___ps->handler_sfun_conv_error,___marker);
#define ___END_SFUN_SCMOBJ } ___EXT(___kill_sfun_stack_marker) (___marker); } ___EXT(___propagate_error) (___PSP ___err);

#define ___BEGIN_SFUN(proc,decl)___processor_state ___ps=___PSTATE;___SCMOBJ ___marker;decl;___SCMOBJ ___err;___SCMOBJ ___errdata=___FAL;___SCMOBJ ___errhandler=___FAL;if ((___err=___EXT(___make_sfun_stack_marker) (___ps,&___marker,proc))==___FIX(___NO_ERR)) {
...
#define ___SFUN_ERROR while ((___err!=___FIX(___NO_ERR))&&(___err!=___FIX(___UNWIND_C_STACK))) {___ps->temp1=___err;___ps->temp2=___errdata;___ps->temp3=___marker;___ps->temp4=___errhandler;if ((___err=___EXT(___call) (___PSP 0,___ps->handler_sfun_conv_error,___marker))==___FIX(___NO_ERR))
...
#define ___BEGIN_CFUN_CTX ___SET_STK(-___FRAME_STACK_RA,___R0) ___W_ALL {
#define ___END_CFUN_CTX } ___R_ALL

#define ___BEGIN_CFUN_DEF(cproc) \
___HIDDEN void cproc ___P((___processor_state ___ps), () \
___processor_state ___ps;) {
#define ___END_CFUN_DEF }

#define ___R_ALL_INL ___CFUN_SELECT(___R_ALL,)
#define ___W_ALL_INL ___CFUN_SELECT(___W_ALL,)

#define ___BEGIN_CFUN_VOID(cproc) ___BEGIN_CFUN(cproc,)
#define ___END_CFUN_VOID ___END_CFUN

#define ___BEGIN_CFUN_SCMOBJ(cproc) ___BEGIN_CFUN(cproc,___SCMOBJ ___result)
#define ___END_CFUN_SCMOBJ ___END_CFUN

#define ___BEGIN_CFUN(cproc,decl) \
___CFUN_SELECT(___BEGIN_CFUN_CTX,___BEGIN_CFUN_DEF(cproc)) \
___SCMOBJ ___err=___FIX(___NO_ERR); \
___SCMOBJ ___errdata=___FAL; \
___SCMOBJ ___errhandler=___FAL; \
decl; \
___CFUN_SELECT(,___ps->temp1=___FIX(___NO_ERR);)
#define ___END_CFUN \
___CFUN_SELECT(___END_CFUN_CTX,___END_CFUN_DEF)

#define ___CFUN_CONV_ERROR_EXIT_INL \
___ps->temp3=___LBL(0); \
___JUMPEXTPRM(___SET_NARGS(___NARGS),___ps->handler_cfun_conv_error)

#define ___CFUN_CONV_ERROR_EXIT_OOL \
___SET_NARGS(___NARGS) return;

#define ___CFUN_CONV_ERROR \
if ((___err!=___FIX(___NO_ERR))&&(___err!=___FIX(___UNWIND_C_STACK))) { \
  ___ps->temp1=___err; \
  ___ps->temp2=___errdata; \
  ___ps->temp4=___errhandler; \
  ___CFUN_SELECT(___CFUN_CONV_ERROR_EXIT_INL,___CFUN_CONV_ERROR_EXIT_OOL) \
}

#define ___CFUN_OOL(cproc) \
___BEGIN_CFUN_CTX \
cproc (___ps); \
___END_CFUN_CTX \
 if (___ps->temp1 != ___FIX(___NO_ERR)) { \
  ___ps->temp3=___LBL(0); \
  ___JUMPEXTPRM(___NOTHING,___ps->handler_cfun_conv_error) \
}

#define ___BEGIN_CFUN_BODY
#define ___END_CFUN_BODY ___AT_END
#define ___BEGIN_CFUN_BODY_CLEANUP ___W_ALL_INL ___BEGIN_TRY
#define ___END_CFUN_BODY_CLEANUP ___AT_END ___END_TRY ___R_ALL_INL

#define ___BEGIN_CFUN_VOIDold ___BEGIN_CFUN_CTX ___SCMOBJ ___err=___FIX(___NO_ERR);___SCMOBJ ___errdata=___FAL;___SCMOBJ ___errhandler=___FAL;
...
#define ___END_CFUN_VOIDold ___END_CFUN_CTX

#define ___BEGIN_CFUN_SCMOBJold ___BEGIN_CFUN_CTX ___SCMOBJ ___err=___FIX(___NO_ERR);___SCMOBJ ___result;___SCMOBJ ___errdata=___FAL;___SCMOBJ ___errhandler=___FAL;
...
#define ___END_CFUN_SCMOBJold ___END_CFUN_CTX

#define ___BEGIN_CFUNold(decl) ___BEGIN_CFUN_CTX ___SCMOBJ ___err=___FIX(___NO_ERR);___SCMOBJ ___errdata=___FAL;___SCMOBJ ___errhandler=___FAL;decl;
...
#define ___END_CFUNold ___END_CFUN_CTX
...
typedef struct ___rc_header_struct
  {
    ...
    ___WORD refcount; /* integer but declared ___WORD for alignment */
...
/*
 * Custom msection allocator (when not NULL).
 */

___msection *(*custom_msection_alloc_) ___PVOID;
...
/*
 * Header structure of procedural interrupts.
 */

typedef struct ___procedural_interrupt_header_struct
  {
    struct ___procedural_interrupt_header_struct * ___VOLATILE next;
    ___SCMOBJ (*execute_fn) ___P((void *self, ___SCMOBJ op),());
  } ___procedural_interrupt_header;
...
typedef struct ___processor_state_struct
  {
    ___WORD * ___VOLATILE stack_trip; /* place first for fast access */
    ___WORD *stack_limit;
    ___WORD *fp;
    ___WORD *stack_start;
    ___WORD *stack_break;
    ...
    ___WORD type_cache[2];
#define ___CHAR_INPUT_PORT_TYPE_CACHE  0
#define ___CHAR_OUTPUT_PORT_TYPE_CACHE 1
    ...
    /* queue of procedural interrupts */
    ___procedural_interrupt_header * ___VOLATILE procedural_interrupts_head;
    ___procedural_interrupt_header * ___VOLATILE procedural_interrupts_tail;
    ___MUTEX_DECL(procedural_interrupts_mut)

    ___WORD processor_scmobj[___SUBTYPED_BODY+___PROCESSOR_SIZE+___PADDING_SIZE]; /* Scheme processor object */

#ifdef ___USE_UNWIND_C_STACK_TO_TRAMPOLINE
    ___WORD *c_stack_trip;
#endif
    ...
    ___processor_state pstate_binding;
#endif
...
typedef void (*___host) ___P((___processor_state ___ps),());
...
typedef struct ___module_struct
  {
    int version;
    int kind;
    int sizeof_module_struct;
    int sizeof_global_state_struct;
    int sizeof_virtual_machine_state_struct;
    int sizeof_processor_state_struct;
    ___UTF_8STRING name;
    int flags;
...
#define ___IO_SETTINGS_STDIN    0
#define ___IO_SETTINGS_STDOUT   1
#define ___IO_SETTINGS_STDERR   2
#define ___IO_SETTINGS_TERMINAL 3
#define ___IO_SETTINGS_STDIO    4
#define ___IO_SETTINGS_FILE     5
#define ___IO_SETTINGS_GENERAL  6
#define ___IO_SETTINGS_LAST     ___IO_SETTINGS_GENERAL

#ifndef ___IO_SETTINGS_DEFAULT
#define ___IO_SETTINGS_DEFAULT \
...
typedef struct ___global_state_struct
  {
    ...
    void (*___set_standard_level)
       ___P((int level),
            ());
    int (*___get_debug_settings) ___PVOID;
    int (*___set_debug_settings)
       ___P((int mask,
             int new_settings),
            ());
    int (*___get_io_settings)
       ___P((int index),
            ());
    void (*___set_io_settings)
       ___P((int index,
             int settings),
            ());
    ___UCS_2STRING (*___get_gambitdir) ___PVOID;
    void (*___set_gambitdir)
       ___P((___UCS_2STRING gambitdir),
            ());
    ___UCS_2STRING* (*___get_gambitdir_map) ___PVOID;
    void (*___set_gambitdir_map)
       ___P((___UCS_2STRING *gambitdir_map),
            ());
    ___UCS_2STRING* (*___get_module_search_order) ___PVOID;
    void (*___set_module_search_order)
       ___P((___UCS_2STRING *module_search_order),
            ());
    ___UCS_2STRING* (*___get_module_whitelist) ___PVOID;
    void (*___set_module_whitelist)
       ___P((___UCS_2STRING *module_whitelist),
            ());
    int (*___get_module_install_mode) ___PVOID;
    void (*___set_module_install_mode)
       ___P((int module_install_mode),
            ());
    ___UCS_2STRING (*___get_repl_client_addr) ___PVOID;
    void (*___set_repl_client_addr)
       ___P((___UCS_2STRING repl_client_addr),
            ());
    ___UCS_2STRING (*___get_repl_server_addr) ___PVOID;
    void (*___set_repl_server_addr)
    ...
    ___SCMOBJ (*___run)
       ___P((___PSD
            ...
            ());
#ifdef ___USE_UNWIND_C_STACK_TO_TRAMPOLINE
    void (*___unwind_c_stack_to_trampoline)
       ___P((___PSDNC),
            ());
#endif
#ifdef ___SUPPORT_LOWLEVEL_EXEC
    void (*___lowlevel_exec)
       ___P((___processor_state ___ps),
            ());
#endif
    ...
    void (*___actlog_dump)
       ___P((___virtual_machine_state ___vms,
           ...
           ());
#endif
    void (*___raise_high_level_interrupt_pstate)
       ___P((___processor_state ___ps,
             ___SCMOBJ intr),
            ());
    void (*___init_procedural_interrupt)
       ___P((void *intr,
             ___SCMOBJ (*execute_fn) ___P((void *self, ___SCMOBJ op),())),
            ());
    void (*___raise_procedural_interrupt_pstate)
       ___P((___processor_state ___ps,
             void *intr),
            ());
    ...
    void (*___raise_interrupt)
       ___P((int code),
            ());
    ___SCMOBJ (*___service_interrupts_pstate)
       ___P((___processor_state ___ps),
            ());
    void (*___refresh_interrupts_pstate)
       ___P((___processor_state ___ps),
            ());
...
___IMP_FUNC(void*,___alloc_rc_no_register)
   ___P((___SIZE_T bytes),
        ());
___IMP_FUNC(void*,___register_rc)
   ___P((___PSD
         void *ptr),
        ());
...
___IMP_FUNC(void,___addref_rc)
   ___P((void *ptr),
        ());
___IMP_FUNC(___WORD,___refcount_rc)
   ___P((void *ptr),
        ());
...
___IMP_FUNC(void,___still_obj_refcount_dec)
   ___P((___SCMOBJ obj),
        ());
___IMP_FUNC(___WORD,___still_obj_refcount)
   ___P((___SCMOBJ obj),
        ());
...
___IMP_FUNC(void,___set_standard_level)
   ___P((int level),
        ());
___IMP_FUNC(int,___get_debug_settings) ___PVOID;
___IMP_FUNC(int,___set_debug_settings)
   ___P((int mask,
         int new_settings),
        ());
___IMP_FUNC(int,___get_io_settings)
   ___P((int index),
        ());
___IMP_FUNC(void,___set_io_settings)
   ___P((int index,
         int settings),
        ());
___IMP_FUNC(___UCS_2STRING,___get_gambitdir) ___PVOID;
___IMP_FUNC(void,___set_gambitdir)
   ___P((___UCS_2STRING gambitdir),
        ());
___IMP_FUNC(___UCS_2STRING*,___get_gambitdir_map) ___PVOID;
___IMP_FUNC(void,___set_gambitdir_map)
   ___P((___UCS_2STRING *gambitdir_map),
        ());
___IMP_FUNC(___UCS_2STRING*,___get_module_search_order) ___PVOID;
___IMP_FUNC(void,___set_module_search_order)
   ___P((___UCS_2STRING *module_search_order),
        ());
___IMP_FUNC(___UCS_2STRING*,___get_module_whitelist) ___PVOID;
___IMP_FUNC(void,___set_module_whitelist)
   ___P((___UCS_2STRING *module_whitelist),
        ());
___IMP_FUNC(int,___get_module_install_mode) ___PVOID;
___IMP_FUNC(void,___set_module_install_mode)
   ___P((int module_install_mode),
        ());
___IMP_FUNC(___UCS_2STRING,___get_repl_client_addr) ___PVOID;
___IMP_FUNC(void,___set_repl_client_addr)
   ___P((___UCS_2STRING repl_client_addr),
        ());
___IMP_FUNC(___UCS_2STRING,___get_repl_server_addr) ___PVOID;
___IMP_FUNC(void,___set_repl_server_addr)
   ___P((___UCS_2STRING repl_server_addr),
        ());
...
___IMP_FUNC(___SCMOBJ,___run)
   ___P((___SCMOBJ thunk),
        ());
#ifdef ___USE_UNWIND_C_STACK_TO_TRAMPOLINE
___IMP_FUNC(void,___unwind_c_stack_to_trampoline)
   ___P((___PSDNC),
        ());
#endif
#ifdef ___SUPPORT_LOWLEVEL_EXEC
___IMP_FUNC(void,___lowlevel_exec)
   ___P((___processor_state ___ps),
        ());
#endif
...
___IMP_FUNC(void,___actlog_dump)
   ___P((___virtual_machine_state ___vms,
        ...
        ());
#endif
___IMP_FUNC(void,___raise_high_level_interrupt_pstate)
   ___P((___processor_state ___ps,
         ___SCMOBJ intr),
        ());
___IMP_FUNC(void,___init_procedural_interrupt)
   ___P((void *intr,
         ___SCMOBJ (*execute_fn) ___P((void *self, ___SCMOBJ op),())),
        ());
___IMP_FUNC(void,___raise_procedural_interrupt_pstate)
   ___P((___processor_state ___ps,
         void *intr),
        ());
...
___IMP_FUNC(void,___raise_interrupt)
   ___P((int code),
        ());
___IMP_FUNC(___SCMOBJ,___service_interrupts_pstate)
   ___P((___processor_state ___ps),
        ());
___IMP_FUNC(void,___refresh_interrupts_pstate)
   ___P((___processor_state ___ps),
        ());
...
___IMP_FUNC(___SCMOBJ,___thread_init_from_self)
   ___P((___thread *thread),
        ());
___IMP_FUNC(___SCMOBJ,___thread_create)
   ___P((___thread *thread),
        ());
___IMP_FUNC(___SCMOBJ,___thread_join)
   ___P((___thread *thread),
        ());
___IMP_FUNC(void,___thread_exit) ___PVOID;
#ifdef ___USE_emulated_sync
